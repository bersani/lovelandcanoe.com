...
BEGIN
{
  ...
  $allow_empty_ref   = 1;
  $max_recipients    = 3;
  $mailprog          = 'SMTP:mail.lovelandcanoe.com'; #'/usr/local/bin/sendmail -t'; # the -oi seems to be a problem
  $postmaster        = 'Rob@AgileConsultingLLC.com';
  @referers          = qw(lovelandcanoe.com 8.17.86.55  localhost);
  @allow_mail_to     = qw(mbersani@cinci.rr.com rbersani@cinci.rr.com
                          5132905958@messaging.sprintpcs.com 5132954739@vtext.com
                          rob@agileconsultingllc.com localhost);
  @recipients        = ();
  %recipient_alias   = ('m' => 'mbersani@cinci.rr.com',
                        'r' => 'rbersani@cinci.rr.com',
                        't' => '5132905958@messaging.sprintpcs.com', # '5132954739@vtext.com',
                        'a' => 'rob@agileconsultingllc.com');
...
From: reservation@lovelandcanoe.com (Online Reservation)
Subject: Loveland Canoe and Kayak Reservation
...
sub handle_request {
  ...
  if ($self->{FormConfig}{address} !~ /^http/i) {
    $self->send_main_email($date, $email, $realname);

    $self->{Text_Recipients} = $self->get_text_recipients;
    $self->send_vtext_email($date, $email, $realname);
...
=item get_text_recipients ()

Determines the list of configured text_recipients from the form inputs and the
C<recipient_alias> configuration setting, and returns them as a list.

Sets the C<Hide_Recipient> field in the blessed hash to a true value if
one or more of the text_recipients were aliased and so should be hidden to
foil address harvesters.

=cut

sub get_text_recipients {
  my ($self) = @_;

  my $text_recipient = $self->{FormConfig}{text_recipient};
  my @text_recipients;

  if (length $text_recipient) {
    foreach my $r (split /\s*,\s*/, $text_recipient) {
      if (exists $self->{CFG}{recipient_alias}{$r}) {
        push @text_recipients, split /\s*,\s*/, $self->{CFG}{recipient_alias}{$r};
        $self->{Hide_Recipient} = 1;
      }
      else {
        push @text_recipients, $r;
      }
    }
  }

  return \@text_recipients;
}

=item send_vtext_email ( DATE, EMAIL, REALNAME )

Sends the vtext email.  DATE is a date string, EMAIL is the
user's email address if they entered a valid one and REALNAME
is the user's real name if entered.

=cut

sub send_vtext_email {
  my ($self, $date, $email, $realname) = @_;

  my $mailer = $self->mailer;
  $mailer->newmail($self->name_and_version, $self->{CFG}{postmaster}, @{ $self->{Text_Recipients} });

  $self->send_vtext_email_header($email, $realname);
  $mailer->print("\n");
  $self->send_vtext_email_fields;

  $mailer->endmail;
}

=item send_vtext_email_header ( EMAIL, REALNAME )

Sends the email header for the vtext email, not including the terminating
blank line.

=cut

sub send_vtext_email_header {
  my ($self, $email, $realname) = @_;
  ...
  my $to = join ',', @{ $self->{Text_Recipients} };
  my $from = $self->build_from_address($email ,$realname);
$subject = "$self->{Form}{trip} for $self->{Form}{people}";
#X-Mailer: ${\( $self->name_and_version )}
  $self->mailer->print(<<END);
To: $to
From: $from
...
=item send_vtext_email_fields ()

Outputs the form fields to the email body.

=cut

sub send_vtext_email_fields {
  my ($self) = @_;

#   foreach my $f (@{ $self->{Text_Field_Order} }) {
#   foreach my $f (qw( trip date time canoe kayak people realname phone )) {
#     if (defined $self->{Form}{$f}) {
#       $self->mailer->print(substr($f,0,2) . " $self->{Form}{$f}\n");
#     }
#   }
  $self->mailer->print(<<TXT);

$self->{Form}{date} : $self->{Form}{time}
$self->{Form}{canoe} / $self->{Form}{kayak}
$self->{Form}{realname}
$self->{Form}{phone}
TXT
}
